...
export function useProofOfLifeStep() {
    ...
    const addNewGroup = useCallback((item: ConfiguredAdapter) => {setListOfAdapters(previousList => [...previousList, [item]])}, [setListOfAdapters]);

    const addToGroup = useCallback((item: ConfiguredAdapter, groupIndex: number) => {
    ...
    const removeFromGroup = useCallback((groupIndex: number, itemIndex: number) => {
        setListOfAdapters(previousList => {
            const newList = [...previousList];
            newList[groupIndex] = [...newList[groupIndex]];
    ...
    const updateGroupItem = useCallback((item: ConfiguredAdapter, groupIndex: number, itemIndex: number) => {
        setListOfAdapters(previousList => {
            const newList = [...previousList];
            newList[groupIndex] = [...newList[groupIndex]];
